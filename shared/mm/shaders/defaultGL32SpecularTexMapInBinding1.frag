...
layout ( binding = 1 ) uniform sampler2D TexUnit1SpecularMap;
...
void doADS( int lightIdx, inout vec4 ambient, inout vec4 diffuse, inout vec4 specular )
{
   ...
   vec4 specColor = texture( TexUnit1SpecularMap, TexCoord );
   specular += specColor /*Material.Ks*/ * spec * Lights.Irgba[lightIdx].rgba;
}
