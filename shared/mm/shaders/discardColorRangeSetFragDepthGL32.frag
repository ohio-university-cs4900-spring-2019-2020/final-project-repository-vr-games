#version 430
//Written by Scott Nykl. Discard Shader for a Core 3.2+.
//Given input uniform colorToMatch and an epsilon. All colors in the r,g,b channels that
//do NOT reside within color.rgb +/- epsilon are discarded.
//This shader does not perform any lighting / shading. The color is is the modulation 
//of the input TextureCoord value * the interpolated Vertex Color (normally all white).
...
//   FragColor = color;
	...
	vec4 texel = texture2D( TexUnit0, TexCoord );
	if( texel.x < colorToMatch.x + epsilon && texel.x > colorToMatch.x - epsilon &&
	    texel.y < colorToMatch.y + epsilon && texel.y > colorToMatch.y - epsilon &&
		 texel.z < colorToMatch.z + epsilon && texel.z > colorToMatch.z - epsilon )
	{
		discard;
	}
	else
	{
		FragColor.rgb = texel.rgb * colorToMatch;
      FragColor.a = 1.0;
