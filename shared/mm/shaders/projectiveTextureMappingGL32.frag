...
in vec4 ProjTexCoord; // Projection Texture Coordinate for this vertex
...
void doADS( int lightIdx, inout vec4 ambient, inout vec4 diffuse, inout vec4 specular )
{
   ...
   if( Lights.PosEye[lightIdx].w > 0.5 ) //if this light is a POINT Light (it has a position), then perform point light computations
...
void main()
{
   ...
   //Perform the projective texture mapping
   vec4 projectiveColor = vec4(0);
   if( ProjTexCoord.z > 1.0 )
      projectiveColor = textureProj( TexUnit1, ProjTexCoord );
   
  
   vec4 colorWithProjection = ( vec4(color.xyz * 0.5, 1 ) ) + projectiveColor;
   
   FragColor = colorWithProjection;
}
